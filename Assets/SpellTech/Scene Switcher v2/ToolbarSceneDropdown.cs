...
namespace SpellTech.SceneSwitcherV2
{
        ...
        public static class ToolbarSceneDropdown
    {
        ...
        private static void ShowDropdown(Rect rect)
        {
            ...
            List<SceneAsset> bookmarkedScenes = SceneSwitcherToolWindowV2.LoadBookmarkedScenesFromPrefs();
            ...
            menu.AddSeparator("");
            menu.AddItem(new GUIContent("Add Current Scene to Bookmarks"), false, () => SceneSwitcherToolWindowV2.AddCurrentSceneToBookmarksStatic());
            menu.AddSeparator("");
            menu.AddItem(new GUIContent("Open Scene Switcher V2..."), false, () => SceneSwitcherToolWindowV2.ShowWindow());
    ...
    // Extra toolbar shortcut button: one-click add current scene to bookmarks.
    public static class ToolbarSceneBookmarkButton
    {
        [MainToolbarElement("SpellTech.SceneSwitcher/AddBookmark", defaultDockPosition = MainToolbarDockPosition.Right)]
        public static MainToolbarElement CreateBookmarkButton()
        {
            var icon = EditorGUIUtility.IconContent("d_Toolbar Plus").image as Texture2D;
            var content = new MainToolbarContent("+", icon, "Add Current Scene to Bookmarks");
            return new MainToolbarButton(content, () => SceneSwitcherToolWindowV2.AddCurrentSceneToBookmarksStatic());
        }
    }
#else
    ...
    public static class ToolbarSceneDropdown
    {
        ...
        private static void InjectButton()
        {
            ...
            var bookmarkBtn = buttonRoot.Q<Button>("scene-switcher-toolbar-bookmark-btn");
            var manageBtn = buttonRoot.Q<Button>("scene-switcher-toolbar-manage-btn");
            ...
            // One-click add current scene
            if (bookmarkBtn != null)
                bookmarkBtn.clicked += SceneSwitcherToolWindowV2.AddCurrentSceneToBookmarksStatic;

            // Open the full window
            if (manageBtn != null)
                manageBtn.clicked += () => SceneSwitcherToolWindowV2.ShowWindow();
